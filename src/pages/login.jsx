              ...
              const submit = () => {
                const getuserDetails = async () =>{
                    try{
                        ...
                        const reponse = await fetch("https://mart-backend-1.onrender.com/login", options);
            ...
            return (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center pt-10 sm:pt-20 z-50">
                        <div className="bg-white shadow-lg p-4 sm:p-6 rounded-lg flex flex-col items-center gap-4 sm:gap-6 w-[90vw] max-w-xs sm:max-w-md">
                            ...
                            <p>you don't have accout ? <Link className='text-blue-500' onClick={()=>(setAute(false))}>registor</Link></p>
                        </div>
                </div>
            )
        ...
        const submit = () => {
                const getuserDetails = async () =>{
                    try{
                        ...
                        const reponse = await fetch("https://mart-backend-1.onrender.com/register", options);
                        const data= await reponse.json()
                        console.log(data);
            ...
            return (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center pt-10 sm:pt-20 z-50">
                        <div className="bg-white shadow-lg p-4 rounded-lg flex flex-col items-center gap-2 w-[90vw] max-w-xs sm:max-w-md">
                            ...
                            <p className="text-red-500 text-xs sm:text-sm">{result}</p>
                            <button className="px-4  py-2 bg-blue-500 text-white rounded w-full" value={password} onClick={submit}>Login</button>

                            <p>you already have accout ? <Link className='text-blue-500' onClick={()=>(setAute(true))}>Signup</Link></p>
                        </div>
                </div>
            )
